...
<c:url var="formActionUrl" value="/ui/ajax/admin/user/modalEdit/${user.id}" />
...
<c:url var="usernameCheckUrl" value="/ui/ajax/user/usernameAvailable?update=${user.username}" />
	...
	<form:form commandName="user" class="modalForm" action="${formActionUrl}">
		...
		<div class="form-item">
			<label>Active <span class="color-red">*</span> <form:errors path="active" /></label>
			<%-- <form:input path="active" cssClass="form-control margin-bottom-20" required="required"/> --%>
			<form:select path="active" items="${activeFlags}" cssClass="form-control margin-bottom-20"></form:select>
		</div>
