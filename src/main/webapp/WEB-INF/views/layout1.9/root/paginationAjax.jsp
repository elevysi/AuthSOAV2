...
<div class="tag-box tag-box-v7 text-justify">
	...
	<c:if test="${not empty page.current && not empty totalPages}">
			...
			<c:choose>
					<c:when test="${page.current - noLinksPerSide >= firstDisplay}">
						<c:forEach begin="${page.current -  noLinksPerSide}" end="${page.current - 1}" var="val">
						    <li><a href="<c:out value="${baseUrl}${val}"/>" class="loadPageBtn"><c:out value="${val}"/></a></li>
						</c:forEach>
					</c:when>
		...
		<li class="active"><a href="<c:out value="${baseUrl}${page.current}" />" class="loadPageBtn"><c:out value="${page.current}" /></a></li>
	...
	<c:if test="${page.current < totalPages}" >
		<li><a href="<c:out value='${baseUrl}${page.current + 1}'/>" class="loadPageBtn">»</a></li>
	</c:if>
	</ul>
	</div>
